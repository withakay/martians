...
static inline bool inside(int x,int y,int mx,int my){
    return 0<=x && x<=mx && 0<=y && y<=my;
}

static inline pair<int,int> step(char d){
    if(d=='N') return {0,1};
    if(d=='E') return {1,0};
    if(d=='S') return {0,-1};
    if(d=='W') return {-1,0};
...
static inline char leftc(char d){
    if(d=='N') return 'W';
    if(d=='W') return 'S';
    if(d=='S') return 'E';
    if(d=='E') return 'N';
...
static inline char rightc(char d){
    if(d=='N') return 'E';
    if(d=='E') return 'S';
    if(d=='S') return 'W';
    if(d=='W') return 'N';
...
int main(){
    ...
    auto has = [&](int x,int y,char d){
        for(auto &s:scents) {
            if(s.x==x&&s.y==y&&s.d==d) return true;
        }
        return false;
    };
    for(size_t i=1;i+1<lines.size();i+=2){
        int x,y; char d; {
            stringstream ss(lines[i]); ss>>x>>y>>d;
        }
        ...
        for(char c: instr){
            ...
            if(c=='L')
            {
                ...
                if(!inside(nx,ny,mx,my)){
                    if(has(x,y,d)) continue;
                    scents.push_back({x,y,d});
                    lost=true;
                } else {
                    x=nx; y=ny;
                }
