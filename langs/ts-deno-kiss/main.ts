...
if (input.trim().length === 0) err("No input provided");
...
try {
  const lines = input
    ...
    .map((s) => s.trim())
    .filter(Boolean);
  if (lines.length === 0) err("Empty input");
  const head = lines[0].split(/\s+/);
  if (head.length < 2) err("Invalid header");
  ...
  // Write trailing blank line to match goldens
  Deno.stdout.write(new TextEncoder().encode(out.join("\n") + "\n\n"));
} catch (e) {
